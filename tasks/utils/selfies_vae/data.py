...
class SELFIESDataset(Dataset):
    ...
    def encode(self, smiles):
        encoded = []
        for s in [*smiles, '<stop>']:
            if s in self.vocab2idx:
                token = self.vocab2idx[s]
                encoded.append(token)
            else:
                print(f"unrecognized token: {s}")
        # return torch.tensor([self.vocab2idx[s] for s in [*smiles, '<stop>']]) # KeyError: '[#Ring2]' ... 
